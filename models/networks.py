...
def init_weights(net, init_type='normal', init_gain=0.02):
    ...
    def init_func(m):  # define the initialization function
        ...
        if hasattr(m, 'weight') and (classname.find('Conv') != -1 or classname.find('Linear') != -1): #hasattr判断是否包含相应属性，有conv或者linear
            if init_type == 'normal':
                init.normal_(m.weight.data, 0.0, init_gain)#初始化,均值0.0,std为0.02
                ...
                raise NotImplementedError('initialization method [%s] is not implemented' % init_type) #没有实施初始化
            if hasattr(m, 'bias') and m.bias is not None:
                init.constant_(m.bias.data, 0.0) #给偏执初始化,常数初始化
        elif classname.find('BatchNorm2d') != -1:  # BatchNorm Layer's weight is not a matrix; only normal distribution applies.
            init.normal_(m.weight.data, 1.0, init_gain) #继续初始
            init.constant_(m.bias.data, 0.0)

    print('initialize network with %s' % init_type)
    net.apply(init_func)  # apply the initialization function <init_func> #递归调用初始化
...
def init_net(net, init_type='normal', init_gain=0.02, gpu_ids=[]):
    ...
    if len(gpu_ids) > 0:
        ...
        net.to(gpu_ids[0])
        net = torch.nn.DataParallel(net, gpu_ids)  # multi-GPUs 多gpu传播
...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02, gpu_ids=[]):
    ...
    Parameters:
        ...
        init_gain (float)  -- scaling factor for normal, xavier and orthogonal.初始化的方式
        gpu_ids (int list) -- which GPUs the network runs on: e.g., 0,1,2
    ...
    if netG == 'resnet_9blocks': #13,1,nor,False,9层
        net = ResnetGenerator(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, n_blocks=9)
    elif netG == 'resnet_9attentionblocks':
        net = ResnetGeneratorAttention(input_nc, output_nc, ngf,attention_choose="CBAM", norm_layer=norm_layer, use_dropout=use_dropout, n_blocks=15)
    elif netG == 'resnet_6blocks':
        net = ResnetGenerator(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, n_blocks=6)
    # elif netG == 'resnet_9blocksCABM':
    #     net = ResnetGeneratorCABM(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, n_blocks=9)
    elif netG == 'unet_64':
...
class GANLoss(nn.Module):
    ...
    def __init__(self, gan_mode, target_real_label=1.0, target_fake_label=0.0):
        ...
        self.register_buffer('real_label', torch.tensor(target_real_label)) #真实的label
        self.register_buffer('fake_label', torch.tensor(target_fake_label)) #虚假的label
        self.gan_mode = gan_mode
        if gan_mode == 'lsgan':
            self.loss = nn.MSELoss() #最小二乘 
...
class ResnetGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, padding_type='reflect'):
        ...
        Parameters:
            ...
            output_nc (int)     -- the number of channels in output images
            ngf (int)           -- the number of filters in the last conv layer
            norm_layer          -- normalization layer  BN,LN,IN,GN,SN
            use_dropout (bool)  -- if use dropout layers
        ...
        model = [nn.ReflectionPad2d(3),#四周填充三行
                 nn.Conv2d(input_nc, ngf, kernel_size=7, padding=0, bias=use_bias),#13,64，再用7的算子size减
                 norm_layer(ngf), #batch normal
                 nn.ReLU(True)]

        n_downsampling = 2 #下采样程度
        for i in range(n_downsampling):  # add downsampling layers
            mult = 2 ** i #2的此方
            model += [nn.Conv2d(ngf * mult, ngf * mult * 2, kernel_size=3, stride=2, padding=1, bias=use_bias),
                      norm_layer(ngf * mult * 2), #128 ,256
                      nn.ReLU(True)]
        #out 256
        mult = 2 ** n_downsampling #4
        for i in range(n_blocks):       # add ResNet blocks #9层
            #256
            model += [ResnetBlock(ngf * mult, padding_type=padding_type, norm_layer=norm_layer, use_dropout=use_dropout, use_bias=use_bias)]

        for i in range(n_downsampling):  # add upsampling layers
            mult = 2 ** (n_downsampling - i) #4
            model += [nn.ConvTranspose2d(ngf * mult, int(ngf * mult / 2), #256 ,128
        ...
        self.model = nn.Sequential(*model) #装入
...
class ResnetGeneratorAttention(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64,attention_choose="SE", norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, padding_type='reflect'):
        """Construct a Resnet-based generator
        ...
        Parameters:
            ...
            output_nc (int)     -- the number of channels in output images
            ngf (int)           -- the number of filters in the last conv layer
            attention_choose SE,ECA,CA,CBAM
            norm_layer          -- normalization layer  BN,LN,IN,GN,SN
            use_dropout (bool)  -- if use dropout layers
            n_blocks (int)      -- the number of ResNet blocks
            padding_type (str)  -- the name of padding layer in conv layers: reflect | replicate | zero
        ...
        # aModel =*[if i == attention_choose for i in ["SE","ECA","CA","CBAM"]else ]
        if attention_choose=="SE":aModel=SE()
        elif attention_choose=="ECA":aModel=ECA()
        elif attention_choose=="CA":aModel=CA()
        elif attention_choose=="CBAM":aModel=CBAM()
        model = [nn.ReflectionPad2d(3),#四周填充三行
                 nn.Conv2d(input_nc, ngf, kernel_size=7, padding=0, bias=use_bias),#13,64，再用7的算子size减
                 norm_layer(ngf), #batch normal
                 nn.ReLU(True)]
...
class NLayerDiscriminatorS(nn.Module):
    ...
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d):
        ...
        if type(norm_layer) == functools.partial:  # no need to use bias as BatchNorm2d has affine parameters 仿射参数
            use_bias = norm_layer.func == nn.InstanceNorm2d
        else:
            use_bias = norm_layer == nn.InstanceNorm2d
        kw = 4
...
class FTGAN_Generator_MLAN(nn.Module):
    def __init__(self, ngf=64, use_dropout=False, n_blocks=6):
        ...
        self.style_encoder = FTGAN_Encoder(input_nc=1, ngf=ngf)
        self.content_encoder = FTGAN_Encoder(input_nc=1, ngf=ngf) #FT编码器,输入维度是1,尺寸64
        self.decoder = FTGAN_Decoder(use_dropout=use_dropout, n_blocks=n_blocks, ngf=ngf)
    ...
    def forward(self, inp):
        content_image, style_images = inp
        B, K, _, _ = style_images.shape #1,12，一个批次，12个维度
        content_feature = self.content_encoder(content_image) #内容编码器
        style_features = self.style_encoder(style_images.view(-1, 1, 64, 64))
