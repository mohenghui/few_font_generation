...
from data.image_folder import make_dataset_update
from PIL import Image
import random

class FontTestDataset(BaseDataset):
    @staticmethod
    def modify_commandline_options(parser, is_train):
        ...
        parser.set_defaults(load_size=64, num_threads=8, display_winsize=64)
        if is_train:
    ...
    def __init__(self, opt):
        ...
        # if opt.dataset_mode=='font_test':
        #     self.content_language='source'
        BaseDataset.__init__(self, opt)
        self.font_list=os.listdir(os.path.join(opt.dataroot, opt.phase, self.style_language))
        self.dataroot = os.path.join(opt.dataroot, opt.phase, self.content_language)  # get the image directory #得到根路径，训练还是测试，内容,获得中文数据
        ...
        self.paths=sorted(make_dataset_update(self.dataroot,opt.font_list,self.font_list,opt.max_dataset_size))
        # print(self.paths)
